...
def control(name, node, obj, lines):
    ...
    def children(node, cls):
        return node.children[0].findall(cls, descend=False, siblings=True, ascend=False)
    # control might have enum options
    idx = node.first_child_matching_class(nodes.field_list)
    if idx is not None:
        for f in node.children[idx].children:
            name = f.next_node(nodes.field_name)
            if(name.astext().strip() == 'enum'):

                o['enum'] = {}

                items_list = f.next_node(nodes.field_body).next_node(nodes.field_list)

                for x in items_list.children[0].findall(
                    nodes.field, siblings=True, descend=False
                ):
                    name = x.next_node(nodes.field_name).astext().strip()
                    val = x.next_node(nodes.field_body).rawsource.strip()
                    o['enum'][name] = val
...
def parse(content):
    ...
    def log_badness(message):
        logging.warning(
            f"{nodes.Element.astext(message)}\n\t{lines[message['line']-1]}")
    ...
    for fl in fieldlists:
        for f in find_children(fl, nodes.field):
            ...
            if parts:
                globals()[parts.group(1)](
                    parts.group(2), content, object, lines)
            else:
                start = f.line
                end = f.next_node(descend=False, ascend=True,
                                  siblings=True).line
                if end - start > 1:
